...
MHEtLive_NeoPixel::MHEtLive_NeoPixel(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 3), pin(p), pixels(NULL)
...
MHEtLive_NeoPixel::~MHEtLive_NeoPixel() {
  if(pixels) free(pixels);
  pinMode(pin, INPUT);
}

void MHEtLive_NeoPixel::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void MHEtLive_NeoPixel::show(void) {
...
// 8 MHz(ish) AVR ---------------------------------------------------------
  ...
  if((type & NEO_SPDMASK) == NEO_KHZ800) { // 800 KHz bitstream
    ...
    // this covers all the pins on the MHEtLive Flora and the bulk of
...
void MHEtLive_NeoPixel::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void MHEtLive_NeoPixel::setPixelColor(
...
void MHEtLive_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t MHEtLive_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t MHEtLive_NeoPixel::getPixelColor(uint16_t n) const {
  if(n >= numLEDs) {
...
uint8_t *MHEtLive_NeoPixel::getPixels(void) const {
  return pixels;
}

uint16_t MHEtLive_NeoPixel::numPixels(void) const {
  return numLEDs;
}
...
void MHEtLive_NeoPixel::setBrightness(uint8_t b) {
...
uint8_t MHEtLive_NeoPixel::getBrightness(void) const {
  return brightness - 1;
}

void MHEtLive_NeoPixel::clear() {
  memset(pixels, 0, numBytes);
}
